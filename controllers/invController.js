...
invCont.addClassification = async function (req, res, next) {
  ...
  if (!validNamePattern.test(classification_name.trim())) {
    req.flash(
      "notice",
...
invCont.addClassification = async function (req, res, next) {
  ...
  if (!validator.isLength(cleanName, { min: 1, max: 30 })) {
    req.flash("notice", "The classification name must be between 1 and 30 characters.");
    return res.redirect("/inv/add-classification");
  }

  if (!validator.matches(cleanName, /^[A-Za-z\s]+$/)) {
    req.flash("notice", "The classification name must only contain letters and spaces.");
    return res.redirect("/inv/add-classification");
  }
